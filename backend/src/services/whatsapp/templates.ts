...
export const MENSAJES = {
  // ==================== BIENVENIDA ====================
  BIENVENIDA: (nombreBarberia: string = 'Madison MVP Barbería') => 
...
👉🏾 Pasado mañana`,
  ...
  HORARIOS_DISPONIBLES: (horarios: Array<{ numero: number; hora: string }>) => {
    let mensaje = `Tengo los siguientes tunos disponibles:\n\n`;
...
export const formatearPrecio = (precio: number): string => {
  return `${precio.toLocaleString('es-CO')} mil pesos`;
};
...
export const generarRadicado = (): string => {
  // Generar código de radicado único (ejemplo: RAD-20251016-A3B7)
